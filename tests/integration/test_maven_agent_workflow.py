...
class TestMavenWorkflowUsesAgent:
    """Test that Maven workflow uses LangGraph create_react_agent()."""
    ...
    async def test_maven_workflow_uses_agent(self):
        ...
        # Mock LangGraph create_deep_agent
        ...
        # LangGraph returns dict with "messages" key containing list of messages
        ...
        with (
            ...
            assert "Maven expert" in create_call.kwargs["prompt"]

            # Verify LLM was created from config
...
class TestMavenWorkflowStoresArtifacts:
    ...
    async def test_maven_workflow_stores_artifacts(self):
        ...
        with (
            ...
            mock_get_tools.return_value = []
            mock_agent = AsyncMock()
            # LangGraph returns dict with "messages" key
            mock_agent.ainvoke.return_value = {"messages": [mock_agent_response]}
            mock_create_agent.return_value = mock_agent
            mock_get_llm.return_value = MagicMock()
...
class TestMavenAgentToolCallRetry:
    ...
    Note: Current implementation lets LangGraph handle tool execution via graph,
    so expected_tools=None is passed to _invoke_agent_with_retry. This means
    tool call validation is done by LangGraph internally, not by our retry logic.
    """
    ...
    async def test_maven_agent_tool_call_retry(self):
        ...
        # Response with tool calls (LangGraph executes via graph)
    ...
    async def test_maven_agent_fails_after_max_retries(self):
        """Test agent succeeds even without tool calls (LangGraph handles tools via graph).

        Note: Since expected_tools=None, no retry on missing tools. The workflow
        completes successfully as long as LangGraph returns a valid response.
        """
        import json

        from src.workflows.maven_maintenance_agent import run_maven_maintenance_with_agent

        # Response without tool calls - still valid for LangGraph
        ...
        with (
            ...
            # Should succeed - LangGraph handles tool execution internally
            result = await run_maven_maintenance_with_agent(
