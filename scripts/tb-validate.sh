...
cd "$TESTBOOST_ROOT"
set +e
python -m testboost validate "$PROJECT_PATH" "$@"
EXIT_CODE=$?
set -e

if [ $EXIT_CODE -ne 0 ]; then
    echo ""
    echo "[TESTBOOST_FAILED:exit_code=$EXIT_CODE:step=validate]"
    echo "CRITICAL: TestBoost command 'validate' failed. Do NOT proceed with this step manually."
fi
