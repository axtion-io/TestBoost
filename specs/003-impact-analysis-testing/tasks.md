...
- [x] T001 Create src/models/ directory structure if not exists
- [x] T002 [P] Create impact model enums in src/models/impact.py (ChangeCategory, RiskLevel, TestType, ScenarioType)
- [x] T003 [P] Create Impact dataclass in src/models/impact.py
- [x] T004 [P] Create TestRequirement dataclass in src/models/impact.py
- [x] T005 [P] Create DiffChunk dataclass in src/models/impact.py
- [x] T006 Create ImpactReport dataclass in src/models/impact_report.py
...
- [x] T007 Implement git diff extraction tool in src/mcp_servers/git_maintenance/tools/diff.py
- [x] T008 Register diff tool in src/mcp_servers/git_maintenance/langchain_tools.py
- [x] T009 [P] Implement diff chunking logic in src/lib/diff_chunker.py (split_by_file, count_lines, chunk_diff)
- [x] T010 [P] Add CRITICAL_KEYWORDS set for risk detection in src/lib/risk_keywords.py
...
- [x] T011 [US1] Create impact_analysis.py workflow skeleton in src/workflows/impact_analysis.py
- [x] T012 [US1] Implement parse_diff() function to extract file-level changes in src/workflows/impact_analysis.py
- [x] T013 [US1] Implement categorize_change() to map file paths to ChangeCategory in src/workflows/impact_analysis.py
- [x] T014 [US1] Implement identify_affected_components() to extract class/method names and potential break points (inputs, outputs, persistence, cross-cutting concerns per FR-003) from diff in src/workflows/impact_analysis.py
- [x] T015 [US1] Implement analyze_impacts() main entry point that orchestrates parsing and categorization; handle empty diff case returning empty impacts list in src/workflows/impact_analysis.py
- [x] T016 [US1] Add chunking support for large diffs (>500 lines) with progress callback in src/workflows/impact_analysis.py
...
- [x] T017 [US2] Create TEST_TYPE_MAPPING dict (ChangeCategory → TestType) in src/workflows/impact_analysis.py
- [x] T018 [US2] Implement select_test_type() that applies test pyramid logic in src/workflows/impact_analysis.py
- [x] T019 [US2] Update analyze_impacts() to set required_test_type on each Impact in src/workflows/impact_analysis.py
...
- [x] T023 [US3] Implement generate_test_requirements() that creates TestRequirement list per Impact in src/workflows/impact_analysis.py
- [x] T024 [US3] Add nominal case generation (1 per impact) in generate_test_requirements()
- [x] T025 [US3] Add edge case generation (1-2 per impact) in generate_test_requirements()
- [x] T026 [US3] Add regression test generation for bug fixes (is_bug_fix=True) in generate_test_requirements()
- [x] T027 [US3] Add invariant test generation for critical business rules in generate_test_requirements()
- [x] T028 [US3] Implement build_impact_report() to assemble ImpactReport with all impacts and requirements in src/workflows/impact_analysis.py
...
- [x] T029 [US5] Add `impact` subcommand to CLI in src/cli/commands/tests.py
- [x] T030 [US5] Implement JSON output with --output flag in src/cli/commands/tests.py
- [x] T031 [P] [US5] Create GitHub Actions workflow example in .github/workflows/impact-check.yml
- [x] T032 [US5] Add exit code logic: 0=all covered, 1=uncovered business-critical impacts in src/cli/commands/tests.py
