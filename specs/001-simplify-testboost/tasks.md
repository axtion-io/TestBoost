...
- [x] T002 Run baseline smoke test: `python -m testboost --help` must list 6 commands (init, analyze, gaps, generate, validate, status) — record output as baseline
...
- [x] T003 Audit `src/core/` — grep for `fastapi`, `sqlalchemy`, `asyncpg`, `src.db`, `src.api` imports; document which files in `src/core/` are safe to delete vs. needed by `testboost`
...
- [x] T006 Audit `src/lib/startup_checks.py` — identify the `check_llm_connection()` function (called by `src/lib/cli.py:393`) and identify any database connectivity check to remove
...
**Goal**: `python -m testboost <command>` works on a machine with no database or Docker, and `poetry install` no longer installs legacy packages.

**Independent Test**: `python -m testboost --help` lists 6 commands without ImportError; `poetry show | grep -E "fastapi|sqlalchemy|asyncpg|uvicorn|alembic"` returns empty.
...
- [x] T016 [US1] Update `pyproject.toml`: remove `fastapi`, `uvicorn`, `sqlalchemy`, `alembic`, `asyncpg` from `[tool.poetry.dependencies]`; remove `psycopg2-binary` from `[tool.poetry.group.dev.dependencies]`; remove `[tool.poetry.scripts]` entry `boost = "src.cli.main:app"`; update `description` to "AI-powered Java test generation CLI (TestBoost)"; set `fail_under = 0` in `[tool.coverage.report]`; update `addopts` in `[tool.pytest.ini_options]` to remove `--cov=src` flags
...
- [x] T018 [US1] Run smoke test: `python -m testboost --help` — must list all 6 commands without ImportError

**Checkpoint**: US1 complete — `python -m testboost` works, legacy packages gone from `pyproject.toml`
...
- [x] T020 [US2] Verify `.env.example` smoke test: copy to `.env.test`, run `python -m testboost status /nonexistent 2>&1` — must fail with a "not found" or usage error, NOT a Pydantic `ValidationError` about missing env vars
...
**Goal**: Static scan of `testboost/` and `src/mcp_servers/` finds zero imports of removed packages; Docker-related MCP servers are deleted.

**Independent Test**: `grep -r "fastapi\|sqlalchemy\|asyncpg\|uvicorn\|alembic" testboost/ src/mcp_servers/ src/lib/ src/agents/ src/workflows/` returns zero matches.
...
- [x] T026 [US3] Run static scan: `grep -r "fastapi\|sqlalchemy\|asyncpg\|uvicorn\|alembic\|docker" testboost/ src/mcp_servers/ src/lib/ src/agents/ src/workflows/` — must return zero matches (excluding deleted directories)
...
- [x] T032 Run full end-to-end smoke test: `python -m testboost init test-projects/BankApp && python -m testboost status test-projects/BankApp` — must create `.testboost/` session files without errors
...
- [x] T035 Commit all changes with message: `refactor: migrate to TestBoost CLI — remove FastAPI, SQLAlchemy, asyncpg, Docker`
...
4. **STOP and VALIDATE**: `python -m testboost --help` works; `poetry install` clean
5. US1 is independently testable and delivers the core value

### Incremental Delivery

1. Phase 1 + 2 → Audit complete, no code changed
2. Phase 3 → `python -m testboost` works without DB/Docker (US1 done)
