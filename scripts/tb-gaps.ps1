...
# Usage: tb-gaps.ps1 <project_path> [--verbose]
param(
...
if ($exitCode -ne 0) {
    Write-Host ""
    Write-Host "[TESTBOOST_FAILED:exit_code=$exitCode:step=gaps]"
    Write-Host "CRITICAL: TestBoost command 'gaps' failed. Do NOT proceed with this step manually."
}
